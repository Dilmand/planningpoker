...
export class JoinerHandler extends BaseHandler {
    async handle(client: WebSocketClient, receivedPayload: any): Promise<void> {
        ...
        console.log(`${handlerType} action received from client ${client.id}:`, receivedPayload);
        
        await this.processMessage(client, receivedPayload, handlerType);
    }
    
    protected async processMessage(client: WebSocketClient, receivedPayload: any, handlerType: string): Promise<void> {
        try {
            const payload = this.validatePayload(receivedPayload);
            if (!payload) {
                await this.sendError(client, `Invalid ${handlerType} payload. Required fields: action, roomId`);
                return;
            }
            
            await this.processAction(client, payload);
        } catch (error: any) {
            await this.sendError(client, `Error processing ${handlerType} action: ${error.message}`);
        }
    }
    ...
    protected async processAction(client: WebSocketClient, payload: any): Promise<void> {
    ...
    private async processJoinerAction(client: WebSocketClient, payload: JoinerPayload): Promise<void> {
        const actionHandlers: Record<JoinerPayload["action"], (client: WebSocketClient, payload: JoinerPayload) => Promise<void>> = {
    ...
    private async processJoinRoom(client: WebSocketClient, payload: JoinerPayload): Promise<void> {
        const validation = this.validateRequiredFields(payload, ['userName', 'roomId']);
        if (!validation.isValid) {
            console.log(`${validation.missingField} is required for join action from client ${client.id}`);
            await this.sendError(client, `${validation.missingField} is required for join action`);
            return;
        }
        ...
        if (joined) {
            ...
            await this.sendError(client, `Failed to join room ${payload.roomId}`);
        }
    }
    
    private async processLeaveRoom(client: WebSocketClient, payload: JoinerPayload): Promise<void> {
